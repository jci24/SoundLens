...
- Undefined evaluative requests such as “which is best?” are stopped before model or tool execution. SoundLens asks for a metric plus direction, target, or reference instead of treating loudness, peak level, crest factor, or clipping as an overall quality score.
...
- Nontrivial turns show only closed-template observable system activity. Repeated calls to the same evidence tool are summarized in one accumulated activity row. Simple general definitions, direct deterministic RMS, peak, or clipping answers, and criterion clarification remain trace-free; prompts, tool arguments and results, internal identifiers, measurements, raw model output, and private chain-of-thought are never exposed.
