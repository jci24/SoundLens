...
- Questions that explicitly require current information, external sources, standards, products, research, or industry practice route to a bounded web-research path. Organization-plus-practice questions such as how manufacturers test, evaluate, validate, or benchmark product sound are included without treating explicit selected-workspace questions as external research. The path uses hosted web search and returns visible validated HTTP(S) citations. A timeout, network interruption, throttling response, or provider 5xx may trigger one bounded retry; invalid requests, authentication failures, caller cancellation, malformed output, and unsafe citation metadata never retry and continue to fail closed.
