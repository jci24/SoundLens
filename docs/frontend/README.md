...
- The Copilot composer exposes Auto, Workspace, and General context modes. Auto is the default; explicit signal mentions force Workspace. General submissions omit recording, signal, comparison, aligned-pair, and ROI identifiers rather than relying on the backend to ignore frontend-attached evidence.
- The composer states whether workspace context is available or intentionally ignored. Completed turns are labelled `Workspace evidence` or `General knowledge`, and Re-run preserves the original mode and identifier snapshot.
- General knowledge is not SoundLens evidence and currently has no live web search or external citations. Evidence badges, limitations, and tool traces remain workspace-only concepts.
