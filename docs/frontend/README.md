...
- Copilot requests use a POST SSE client so nontrivial backend activity can update live while answer prose remains atomic. Each turn owns an upserted sequence-based trace, the active request is aborted on reset or route unmount, and Re-run clears the old trace while retaining the original identifier snapshot.
