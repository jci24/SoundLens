...
- The user-facing assistant name is `Sona`. Shell and Evidence triggers share one inline, `currentColor` waveform SVG, icon-only presentation, dynamic `Open Sona`/`Close Sona` accessible names, and a keyboard/hover tooltip. Internal feature and contract names continue to use Copilot to avoid unrelated architectural churn.
- Sona navigation suggestions render as explicit confirmation buttons, never automatic redirects. The client strictly validates the additive action contract, checks local recording availability, submits the closed action ID for backend approval, records the returned approval event on the originating turn, and delegates the validated route transition to the application shell. It never executes arbitrary URLs or changes evidence, ROI, playback, or A/B state.
...
- Metric-card activation and the clearly labelled `Evidence & limitations` disclosure open a non-modal right-side inspector instead of expanding evidence inline. The inspector renders only backend-owned comparison values, scope, aligned-pair evidence, coverage, integrity checks, limitations, and collapsed analysis-method and provenance disclosures.
- The workspace shows one compact comparison-context summary. Detailed sample-rate, time-scope, alignment, and calibration states remain flat rows in the existing inspector; the frontend validates their closed contract and never derives a readiness score or standards-compliance conclusion.
- The client strictly validates `comparison-analysis-v1`, its scope, difference convention, aggregate contract, fixed method order, units, identifiers, and versions before disclosure. It never derives or submits methodology.
- The client also strictly validates `comparison-provenance-v1`, its known implementation and decoder versions, SHA-256 formats, method order, scope agreement, and closed limitation set. The collapsed `Provenance` disclosure shows shortened monospaced fingerprints with accessible copy actions for the complete values; clipboard failure leaves the inspector usable and exposes an explicit error.
...
- Completed turns do not display internal routing-mode badges. `answerMode` remains an internal trust and Re-run contract; visible evidence sections and external citations communicate the source information users need.
- Copilot requests use a POST SSE client so nontrivial backend activity can update live while answer prose remains atomic. Each turn owns an upserted sequence-based trace and its original identifier-only request snapshot. New requests include at most six successful completed prose turns; loading, failed, evidence, citation, tool, and activity payloads are excluded. The active request is aborted on reset or route unmount, and Re-run removes later dependent turns before executing the original request and history snapshot.
- The current activity step remains visible through a polite live region. Details are collapsed by default and become `Prepared with N steps` or a stopped summary after completion. Empty traces render nothing, and the older tool-count disclosure is suppressed when the trace already presents tool activity.
- Activity is disclosed as `How this answer was prepared`, never reasoning, thoughts, or routing mode. Model-backed general answers use the same preparation surface as investigation paths. The frontend renders backend-authored titles and summaries but receives no private chain-of-thought, prompts, tool arguments, raw tool results, or partial unvalidated answers.
...
- Guidance responses may include a strictly validated `preview` investigation plan. The Copilot renders it as a collapsed, flat definition list with objective, scope, ordered dependencies, capability policy, evidence requirements, completion criteria, cost, approval metadata, and a shortened reference. The frontend never derives policy, converts the preview into executable controls, or shows a plan for unrelated turns.
- Web-research turns render backend-validated citations as visible links beside supported claims and in a deduplicated Sources list. A collapsed `Source details` disclosure shows the backend-owned publisher host, conservative source class, and explicit access/applicability status. The client strictly validates this additive metadata but never derives source quality or applicability. Web turns do not render SoundLens evidence badges; measurements and workspace tool traces remain workspace-only concepts.
...
- Selected-comparison answers may also include typed metric and signal-finding observations. The client strictly validates their discriminant, status, units, scope, payload, and matching evidence reference before rendering a collapsed `Measured evidence` disclosure. It formats backend values but never recomputes measurements, status, limitations, or reference identity.
