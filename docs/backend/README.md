...
Clear requests for analysis guidelines are handled by a deterministic acoustic-investigation guidance responder before measurement or model routing. It provides a decision-led product-sound workflow, performs no measurement, cites no workspace evidence, and prevents methodology questions from being reduced to the currently selected metric or interpreted as music-production advice.
