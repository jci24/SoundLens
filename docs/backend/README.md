...
Clear RMS, peak-amplitude, and clipping definition questions bypass deterministic measurement intent before Auto routing. Evidence references such as `this signal`, `selected`, `difference`, `compare`, `between`, `channel`, `level of`, or `amplitude of` keep the request eligible for workspace resolution. This prevents an attached A/B pair from turning a theory question into an unrelated measurement answer.
...
`SelectedComparisonOrchestrator` owns that selected-context pipeline: it resolves backend evidence through `IComparisonExplanationContextResolver`, applies deterministic trust guards, acquires the model only when those guards decline, builds the bounded prompt, validates structured output, and restores backend-owned evidence, limitations, and next steps. `AgentQueryHandler` delegates this path while retaining generic tool-calling and translating resolution failures into the existing request error contract.
