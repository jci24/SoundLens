...
`WebResearchResponder` is a separate question-only path over the OpenAI Responses API hosted `web_search` tool. It caps tool calls and output size, disables response storage, and accepts only nonempty plain-text answers with bounded, in-range HTTP(S) citation annotations covering each substantive paragraph. Replacement characters, model-authored Markdown links, uncited claims, missing citations, unsafe URLs, or failed research produce an explicit unavailable response instead of falling back to unsourced model knowledge. It returns `answerMode: web`, external citations, and no SoundLens evidence badges. Hybrid workspace-plus-web synthesis and multi-step deep research remain deferred.
