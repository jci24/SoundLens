...
The response also carries `comparison-analysis-v1`, a closed backend-owned method specification. It fixes the metric order, method identifiers and versions, units, A-minus-B convention, aggregate statistics, and whether the current computation covers full duration or a selected ROI. Reports and the frontend may disclose this contract but cannot submit or modify it. This is method transparency, not complete reproducibility: durable input hashes, implementation/build versions, decoder identity, persisted parameters, and lineage remain deferred.
