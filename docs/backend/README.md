...
- `Features/AudioDecoding/Common/WavAudioDecoder` is the single backend binary-interpretation boundary used by import inventory, waveform, and spectrum. Its metadata-only path reads headers without loading sample payloads; its full path owns RIFF traversal, PCM 8/16/24/32-bit and IEEE-float validation, channel deinterleaving, sample normalization, positive full-scale thresholds, cancellation, and optional consumer limits. Waveform retains raw decoded float samples for metrics, while spectrum requests its pre-existing full-scale clamp during decoding. Consumer caches and all downstream DSP remain feature-owned. Waveform and spectrum derive cache keys through one imported-file evidence identity: the stable recording identity plus a backend-verified content fingerprint when available. Spectrum includes that identity in both decoded-recording and full-duration FFT-point keys, while FFT length remains an independent cache dimension.
...
The response also carries `comparison-analysis-v1`, a closed backend-owned method specification. It fixes the metric order, method identifiers and versions, units, A-minus-B convention, aggregate statistics, and whether the current computation covers full duration or a selected ROI. Reports and the frontend may disclose this contract but cannot submit or modify it.

Every successful comparison also carries `comparison-provenance-v1`. `RecordingComparisonProvenanceService` asynchronously hashes the exact active A/B files before decoding and creates a canonical, culture-independent manifest containing the comparison implementation and application build, WAV decoder, full-duration or ROI scope, ordered methods and aligned channel bases, plus parameter and evidence fingerprints. Waveform and spectrum cache identities include the verified content fingerprint so changed active bytes cannot reuse stale decoded or full-duration FFT evidence. Canonical material excludes filenames, recording identifiers, filesystem paths, timestamps, machine identifiers, report titles, and secrets. The same backend-owned manifest is written to Markdown and PDF reports.

The manifest provides current-session traceability, not complete reproducibility. Source retention, durable lineage, calibration certificates, equipment, environment, operating conditions, signatures, reproduction jobs, equivalence tolerances, and audit certification remain unavailable.
